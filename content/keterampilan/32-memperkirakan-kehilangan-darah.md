---
title: Memperkirakan Kehilangan Darah Postpartum
tags:
  - keterampilan
  - persalinan
  - skdi-4a
...
aliases:
  ...
  - Penilaian Perdarahan Pasca Salin
  - Metode Visual vs Weighted
  - estimasi darah
  - hitung perdarahan
  - blood loss estimation
  - pph assessment
...
> Dokter umum harus **mahir** memperkirakan kehilangan darah postpartum secara akurat, mampu membedakan perdarahan fisiologis dari perdarahan patologis (PPH), dan mengambil keputusan tata laksana berdasarkan estimasi volume darah yang hilang.
...
Keterampilan ini sangat terkait dengan Postpartum Pemeriksaan Fundus (menilai kontraksi uterus sebagai penyebab tersering PPH) dan Kompresi Bimanual (tindakan darurat saat PPH atonia uteri tidak terkendali).
...
```mermaid
flowchart TD
    A["Perdarahan tampak"] --> B{"Metode apa tersedia?"}
    B -->|"Timbangan ada"| C["Timbang semua kain & kasa basah"]
    B -->|"Timbangan tidak ada"| D["Hitung jumlah kasa & handuk basah\n× kapasitas tampung"]
    C --> E["Koreksi air ketuban / irrigasi"]
    D --> E
    E --> F["Tambahkan estimasi visual\n(darah di lantai, celemek, sarung tangan)"]
    F --> G{"Volume > 500 mL?"}
    G -->|"Ya"| H["▶ AKTIFASI TATA LAKSANA PPH"]
    G -->|"Tidak"| I["Lanjutkan observasi\nKala IV: 2 jam"]
    H --> J{"Volume > 1000 mL?"}
    J -->|"Ya"| K["▶ PPH Berat — Siapkan\nTransfusi + Operatif + Rujuk"]
    J -->|"Tidak"| L["Terapi medikamentosa\nOksitosin + misoprostol + KBE"]
    K --> M["Aktivasi CODE MERAH\nTim multidisiplin"]
```
...
1. **Tampung semua darah** — gunakan kain alas partus yang lebar, pasang _underpad_ anti-bocor, dan ember penampung di bawah perineum (posisi lithotomi).
...
| Waktu           | Tindakan                                                                                                       |
| --------------- | -------------------------------------------------------------------------------------------------------------- |
| **0–15 menit**  | Timbang kain alas pertama. Ukur TD, nadi, respirasi. Palpasi fundus (Postpartum Pemeriksaan Fundus).           |
...
> - Total >1000 mL → aktivasi CODE MERAH, siapkan transfusi, operatif (Kompresi Bimanual, balon tamponade, atau laparotomi)
...
- _Cognitive bias_ — kita _ingin_ percaya bahwa perdarahan tidak sebanyak itu
...
>
> 1. Palpasi fundus — apakah uterus membesar? → Postpartum Pemeriksaan Fundus
> 2. Eksplorasi vagina + serviks — laserasi tersembunyi?
> 3. Kompresi bimanual — uji apakah atonia? → Kompresi Bimanual
...
ESTIMASI PERDARAHAN (mL) + SYOK INDEX (SI)
   ...
   │ SI 0,7–1 │     ☐ Atonia uteri → KBE + oksitosin + Kompresi Bimanual
   └──────────┘     ☐ Laserasi → jahit
...
3. **PPH akibat atonia uteri** — bekerja sama erat dengan Kompresi Bimanual.
