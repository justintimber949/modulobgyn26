...
- **Dasar Subspesialisasi**: Fondasi hysteroscopy, MIOMectomy histeroskopik, endometrial ablation, IVF (embryo transfer).

Keterampilan **dilatasi bertahap (Hegar/Pratt), kuretase tajam vs tumpul (sharp vs blunt), orientasi anatomis (fundus, cornu, isthmus), deteksi perforasi** adalah **wajib dokter umum**.
...
4. **Keguguran Tidak Lengkap (Incomplete Abortion)**: Produk konseksi retensi (jika misoprostol gagal/kontrendikasi).
5. **Mola Hidatidosa**: Evakuasi molar (wajib kuretase tajam + oksitosin).
...
**Alat Utama (Set Kuretase Steril):**
| No | Alat | Keterangan |
|----|------|------------|
| 1 | **Dilator Hegar / Pratt** | Set 3-14 mm (Hegar) atau 13-27 Fr (Pratt). **Hegar lebih aman untuk pemula**. |
| 2 | **Kuret Tajam (Sharp Curette)** | Simmonds / Novak / Thomas — ujung tajam, berbagai ukuran (Kecil 8-10mm, Sedang 11-12mm, Besar 13-14mm). |
| 3 | **Kuret Tumpul (Blunt/Suction Curette)** | Karman / MVA cannula (4-12 mm) untuk evakuasi produk konsepsi (lebih cepat, trauma < kuret tajam). |
| 4 | **Tenakulum / Vulsellum** | Pemegang serviks (stabilisasi uterus). Pilih tenakulum single-tooth (Allis) atau double-tooth (Teale). |
| 5 | **Spekulum Cusco** | Ukuran sesuai (kecil/sedang/besar). |
| 6 | **Sond Uterin (Uterine Sound)** | Panjang 30-35 cm, gradasi cm. **WAJIB** sebelum dilatasi. |
| 7 | **Anestesi**: Spuit 10-20 mL, Jarum 23-25G, Lidokain 1% / 2% (blok paraservikal). |
| 8 | **Antiseptik**: Povidon Iodin 10% (vagina/serviks), Alkohol 70% (kulit). |
| 9 | **Kasa Steril, Handuk, Cawan Nier, Pinces**. |
| 10 | **Wadah Spesimen + Formalin 10%** (Histopat) + **Medium Transport** (Kultur/PCR jika perlu). |
| 11 | **OKSITOSIN 10-20 IU IV / IM** (profilaksis/terapi atoni pasca kuretase). |
...
5.  **Tenakulum**: Jepit **labium anterius serviks** (pusat/tengah) → tarik **kaudal & sedikit ventral** → uterus jadi antefleksi/anteversi (mempermudah sondase & dilatasi). **Jangan terlalu kencang** → pendarahan serviks.
6.  **Blok Paraservikal (Anestesi Lokal)**:
    - Jarum 23G, Lidokain 1% 20 mL (10 mL tiap sisi).
    - Titik suntik: **3 jam & 9 jam** pada serviks, kedalaman ±1,5-2 cm (ke jaringan parametrium).
    - **Aspirasi negatif WAJIB** sebelum injeksi (hindari IV).
...
10. **Evaluasi Posisi Uterus**: Antefleksi/Retrofleksi, Anteversi/Retroversi, Lateralisasi. **Penting untuk arah dilatasi & kuretase**.
...
19. **Teknik Kuretase SISTEMATIS (Polanya: "Mengelilingi Jam")**:
    - **FUNDUS**: Mulai di fundus (ujung atas). Gerak **tekan & tarik (push-pull)** atau **rotasi & tarik** mengikuti kontur fundus.
    - **DINDING ANTERIOR**: Dari fundus turun ke isthmus/serviks anterior.
    - **DINDING POSTERIOR**: Dari fundus turun ke isthmus posterior.
    - **CORNU DEXTER & SINISTER (Siku Kanan & Kiri)**: **PALING RENTAN PERFORASI**. Gerak **pelan, dangkal, jangan dorong ke lateral**. Rasakan "tambak" miometrium.
    - **ISTHMUS / SEGMEN BAWAH**: Hati-hati, miometrium tipis.
20. **Kuretase Tajam**: Gerakan **cekung (scooping)** — ujung kuret menempel endometrium, tarik ke luar. **Jangan "menggaruk" (scratching) ke miometrium** → perforasi.
21. **Kuretase Suction (MVA)**: Masukkan kanula ke fundus, aktifkan vakum, gerakkan **bolak-balik + rotasi** merata ke 4 dinding + 2 cornu. Periksa aspirat: jaringan khorion/vili/embrio/plasenta.
22. **Tanda Selesai (End Point)**:
    - **Kuretase Tajam**: Suara **"Krit-krit" / "Keres" (gritty feel)** = kuret menyentuh miometrium kosong (basalis layer). Jaringan tidak lagi menempel. Uterus terasa "keras & licin" (kontraksi).
    - **MVA**: Aspiran **berwarna merah jernih / tidak ada lagi jaringan putih/coklat** (produk konsepsi). Uterus kontraksi keras.
...
27. **Pengiriman Spesimen**:
    - **Histopat**: Semua jaringan → **Formalin 10% (volume 10x jaringan)**. Label: Nama, RM, Tanggal, Lokasi (Fundus/Anterior/Posterior/Cornu), Indikasi.
    - **Kultur/PCR**: Jika infeksi (TB, Gonore, Chlamydia) → medium transport khusus (bukan formalin).
28. **Observasi 2-4 Jam**: Vital signs, perdarahan vaginal, nyeri, suhu. Boleh pulang jika stabil, HB tidak turun drastis, tidak demam.
29. **Edukasi Pulang**: Istirahat 1-2 hari. Hindari hubungan seks/tampun/kolam 2 minggu. Perdarahan spotting 1-2 minggu normal. Tanda bahaya: perdarahan > pembalut/jam, demam >38°C, nyeri perut hebat, bau busuk → ke IGD.
30. **Kontrol 1-2 Minggu**: Pemeriksaan, hasil histopat, evaluasi komplikasi.
...
1.  **TIDAK MELAKUKAN SONDASE SEBELUM DILATASI** → Dilatasi buta → Perforasi serviks/uterus, dilatarasi arah salah.
2.  **DILATASI TERLALU CEPAT / LOMPAT UKURAN** (misal #4 → #10) → Robekan serviks, incompetence serviks, perforasi.
3.  **ARAH DILATASI & KURETASE TIDAK SESUAI POSISI UTEREUS** (Antefleksi vs Retrofleksi) → Dorong ke anterior/posterior → Perforasi.
4.  **KURETASE TAJAM DI CORNU / FUNDUS DENGAN KERAS** → Perforasi cornu (dekat tuba/ovarium/ureter/iliaca vessels).
5.  **MENGGUNAKAN KURETASE TAJAM UNTUK PRODUK KONSEPSI (KEGUGURAN/MOLA)** → Trauma miometrium berlebih, perdarahan, perforasi. **GUNAKAN MVA / KURETASE TUMPUL**.
6.  **TIDAK MENGIRIM SPESIMEN HISTOPAT / SALAH LABEL / SALAH MEDIA (FORMALIN UNTUK KULTUR)** → Diagnosis terlewat / salah.
7.  **TIDAK MEMBERIKAN PROFILAKSIS ANTIBIOTIK / OKSITOSIN** → Endometritis / Atoni Uteri ↑.
8.  **TIDAK MELAKUKAN KONTROL PASCA (HISTOPAT, KOMPILIKASI)** → Missed Ca Endometrium, Asherman, PID kronik.
9.  **MELAKUKAN D&C PADA KEHAMILAN VITAL (TERMASUK ECTOPIC) TANPA KONFIRMASI** → Keguguran terpilih, kerusakan fertilitas. **KONFIRMASI USG INTRAUTERIN WAJIB**.
10. **DOKUMENTASI TIDAK LENGKAP** (Panjang uterus, ukuran dilatasi, jenis kuretase, volume/penampakan aspirat, komplikasi, spesimen) → Medikolegal rawan.
