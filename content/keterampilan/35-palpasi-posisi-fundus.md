...
```mermaid
flowchart LR
    A["Epigastrium"] --> B["Sapu ke bawah"]
    B --> C{"Teraba massa padat?"}
    C -->|Ya| D["Catat posisi fundus"]
    C -->|Tidak| E["Lanjut hingga simfisis"]
    E --> F{"Ada massa?"}
    F -->|Ya| G["Fundus rendah (normal >hari ke-7)"]
    F -->|Tidak| H["Waspada: atonia total? inversio?"]
```
...
```mermaid
flowchart TD
    A["Fundus ditemukan"] --> B{"Kandung kemih kosong?"}
    B -- Tidak --> C["BAK/kateter → palpasi ulang"]
    C --> B
    B -- Ya --> E{"Konsistensi?"}
    E -- Keras --> F{"Posisi sesuai waktu?"}
    F -- Ya --> G["Involusi baik"]
    F -- Tidak --> H["Curigai RPOC/bekuan/infeksi"]
    E -- Kenyal --> I["Kontraksi suboptimal → observasi"]
    E -- Lembek --> J["ATONIA! → Masase + oksitosin"]
```
